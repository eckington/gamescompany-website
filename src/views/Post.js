...
export default class Post extends Component {
  ...
  componentDidMount() {
    const shortName = this.props.location.pathname.substring(
      this.props.type.length + 2
    );

    const post =
      this.props.type === "review"
        ? reviewsList.find(e => e.shortName == shortName)
        : /* postsList.find(e => e.shortName == shortName) */ null;
    ...
    var disqus_config = function() {
      this.page.url = `https://imedia-r085.netlify.com/#/${
      ...
      return (
        <div className="post container">
          <div className="content">
            <h1>
              {titleToDisplay} <span className="lastword">{lastWord}</span>
            </h1>
